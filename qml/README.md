...
Desktop skeleton in `desktop/`: a Qt Quick `ApplicationWindow` with `File`/`Help` menus and a chart filling the space below the menu bar. The app's actions (Connect, About, Exit) live in `core/qml/AppActions.qml` (the `KomaroCore` QML module) rather than being hardcoded in `desktop/qml/Main.qml`, so `Main.qml` is just a "thin shell" that arranges those shared actions into a `MenuBar`. Connect is wired end-to-end: an editable history combo box (backed by `core/`'s `ConnectionManager`, persisted via `QSettings`) records the selected host, and a controls row (sensors, time range, moving-average window) drives `core/`'s `ChartController` to query InfluxDB and feed the result into `core/qml/SensorChart.qml` — a hand-rolled `Canvas` dual-axis (temperature/humidity) line chart shared with the mobile shell, mirroring `nano/plot_sensor.py`. The sensor picker (`core/qml/MultiMeasurementSelect.qml`, also shared) supports selecting more than one measurement at once, overlaying each as its own temperature/humidity line pair distinguished by dash style, mirroring `nano/plot_multi_sensor.py`'s multi-measurement plotting.
...
On Linux the install also registers the app with the desktop environment: `desktop/komaro-desktop.desktop` (a standard freedesktop.org menu entry) and `desktop/icons/komaro-desktop.svg` (a small dual-line-chart icon in the app's own temperature/humidity colors) install to `share/applications/` and `share/icons/hicolor/scalable/apps/` respectively, so the app shows up with its icon in GNOME's "Show Applications" grid (and equivalents on other desktop environments) once installed to a location on `$XDG_DATA_HOME` (`~/.local/share` by default) - which the default `$HOME/.local` prefix already is.
