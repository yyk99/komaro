...
This choice was made together with recording the touch-gesture decision itself in issue #25, anticipating a future zoom feature - see below for how zoom (#26) actually turned out once implemented.

## Why chart zoom uses double-click, and what that costs the hairline tool

`SensorChart.qml`'s zoom (#26) is double-click to set one corner of a temporary time window, a second double-click to set the other and activate it. Double-click (not drag) was chosen specifically because a plain click-drag was already taken - it's the hairline's own "drag to move" gesture (see above) - and `MouseArea` synthesizes double-click from a touch double-tap the same way it does for taps/drags, so this one implementation again covers mouse and touch without branching, exactly like the hairline does.

**The cost:** to recognize a double-click, Qt has to wait and see whether a second click follows within `Qt.styleHints.mouseDoubleClickInterval` (400ms in this Qt build) before concluding "that was just one click". Since the hairline's single-click toggle lives in the *same* `MouseArea`, it now has to wait out that same interval too - `onReleased` no longer toggles the hairline immediately; it starts a `Timer` of that length, and `onDoubleClicked` cancels the timer if a second click arrives in time. The net effect: the hairline's toggle now has a brief (sub-half-second) delay it didn't have before zoom existed - a deliberate, accepted tradeoff (recorded while the feature was still being scoped in issue #26) rather than an oversight.

**Why this timer is unavoidable in QML specifically, unlike Qt Widgets:** the two toolkits deliver a double-click's events differently. Widgets' event sequence is `MousePress -> MouseRelease -> MouseButtonDblClick -> MouseRelease` - the *second click's press* is never delivered as an ordinary `mousePressEvent()` at all; Qt's platform integration (leaning on the native window system's own double-click compression) replaces it with `mouseDoubleClickEvent()` instead. `MouseArea` gets no such help: it receives the raw press/release stream (or synthesized touch events) directly and does its *own* timer-based pattern matching entirely in QtQuick's C++ implementation, layered on top rather than replacing anything. So both clicks of a QML double-click each deliver a full, independent press/release/`clicked` cycle, and `doubleClicked` fires *afterward*, on top of that - meaning a naive `onClicked` handler would fire twice per double-click, not once. That's precisely why the hairline's tap action can't just live in `onClicked`/`onReleased` unconditionally here the way an equivalent Widgets app could mostly get away with - the deferral timer is doing work Widgets gets for free from the platform.

**Exiting zoom deliberately isn't a third chart gesture.** Both apps' time-range `ComboBox` already represents "what time window am I viewing", so zoom reuses it: `displayText: sensorChart.zoomActive ? qsTr("Zoomed") : currentText` overrides only what's *shown* while zoomed, leaving `currentIndex` (and its `Settings` persistence) completely untouched, and `onActivated` (which fires on every explicit popup selection, even reselecting the currently-active preset - unlike `currentIndexChanged`, which only fires on an actual change) calls `SensorChart.resetZoom()` before reloading. This avoids a fourth gesture in a component that already has three (hairline tap, hairline drag, zoom double-click) fighting for the same input surface.

**Touch note:** double-tap-to-zoom works on Android (verified on a physical tablet) purely because it's the same shared `MouseArea` code as desktop - it was not built as a native pinch gesture. Real pinch-to-zoom (`PinchHandler`, computing a scale factor and centroid, mapping that to a time-window resize) is a materially separate addition and remains a possible follow-up, not part of this pass.
